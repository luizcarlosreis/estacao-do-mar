...
export default function RootLayout({
  ...
  const allMenuItems = [
    ...
    { title: 'Reservas', icon: <Calendar size={18} />, path: '/reservas', roles: ['SUPER_ADMIN', 'SINDICO', 'MORADOR', 'PORTEIRO'] },
                  ...
                  <div className="relative">
                    ...
                    {isDropdownOpen && (
                      <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-slate-100 py-2 z-[60] animate-in fade-in slide-in-from-top-2 duration-200">
                        ...
                        {(user?.role === 'MORADOR' || user?.role === 'SUPER_ADMIN' || user?.role === 'SINDICO') && (
                          <>
                            {user?.role === 'MORADOR' && (
