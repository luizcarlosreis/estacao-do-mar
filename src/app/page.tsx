...
export default function Home() {
  ...
  const allModules = [
    ...
    { title: 'Autorizações', icon: <ShieldCheck size={16} />, path: '/autorizacoes', desc: 'Controle de acesso e uso.', roles: ['SUPER_ADMIN', 'PORTEIRO', 'SINDICO', 'MORADOR', 'CONSELHO'], color: 'text-teal-600', bg: 'bg-teal-50' },
    { title: 'Salão de Festas', icon: <Calendar size={16} />, path: '/reservas', desc: 'Solicitações de reserva do salão.', roles: ['SUPER_ADMIN', 'SINDICO', 'MORADOR', 'PORTEIRO', 'CONSELHO'], color: 'text-blue-600', bg: 'bg-blue-50' },
    { title: 'Cadastro de Usuários', icon: <UserCog size={16} />, path: '/usuarios', desc: 'Gerenciamento de usuários administrativos.', roles: ['SUPER_ADMIN', 'ADMINISTRADORA'], color: 'text-purple-600', bg: 'bg-purple-50' },
    ...
    { title: (user?.role === 'MORADOR') ? 'Solicitação de Reparos' : 'Tarefas', icon: <ListTodo size={16} />, path: '/tarefas', desc: (user?.role === 'MORADOR') ? 'Solicitar manutenção.' : 'Backlog de atividades.', roles: ['SUPER_ADMIN', 'SINDICO', 'MORADOR', 'CONSELHO'], color: 'text-indigo-600', bg: 'bg-indigo-50' },
